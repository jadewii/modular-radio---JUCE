...
public:
    ...
    void paint (juce::Graphics& g) override
    {
        // Draw "VOLUME" label to the left of the knob - use full available space to prevent truncation
        g.setColour (juce::Colours::black);
        g.setFont (juce::Font (juce::FontOptions().withHeight(24.0f)).boldened());
        // Use almost the full width available before the knob starts
        g.drawText ("VOLUME", 0, 0, 180, getHeight(), juce::Justification::centredLeft);
    }

    void resized() override
    {
        // Position knob further right to give more space for text
        // 180px for text + smaller knob to fit: 150x150 instead of 168x168
        knob.setBounds (185, 25, 150, 150);  // Smaller knob, positioned right, centered vertically
    }
